...
class ARIMAEngine:
    ...
    def train(self):
        ...
        if (self.args.arima_type == "VARIMA"):
            ...
            if self.args.seasonal:
                seasonal_order = (self.args.P, self.args.D, self.args.Q, self.args.seasonal_periods)
            else:
                seasonal_order = None
        ...
        if (self.args.arima_type == "VARIMA"):
            ...
            for i, feature_name in pbar:
                ...
                self.logger.info(f"Feature {feature_name} training completed in {feature_duration:.2f} seconds")
            
            pbar.close()
        
        total_end_time = time.time()
        self.logger.info(f"All ARIMA models training completed in {total_end_time - total_start_time:.2f} seconds")
    ...
    def _predict_multivariate(self, n_periods, historical_series):
        ...
        combined_values = np.column_stack([predictions_dict[name] for name in self.feature_names])
    ...
    def evaluate(self, test_set="test"):
        ...
        self.logger.info(f"MSE: {mse_score:.6f}, RMSE: {rmse_score:.6f}, MAE: {mae_score:.6f}, MAPE: {mape_score:.4f}%")
    ...
    def plot_results(self, predictions_df):
        ...
        preds = predictions_df.drop(columns=['Unnamed: 0']).values  # Drop index column and convert to numpy array
        
        # Extract true test values from the test_series
        true_labels = self.test_series.values()  # Assuming test_series is a pandas DataFrame
